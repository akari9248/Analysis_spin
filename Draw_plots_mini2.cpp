...
public:
    ...
    void initialize() override {
        t->Add(type+prefix+".root/DataInfo");
...
int main(){
  vector<string> partontypes = {"Parton", "Hadron"};
  for (auto &partontype : partontypes) {
    EventsAnalyzer analyzerspinon("_Spinon", partontype);
    analyzerspinon.run();
    analyzerspinon.finalize("spin_on_" + partontype + "_3.root");
    EventsAnalyzer analyzerspinoff("_Spinoff", partontype);
    analyzerspinoff.run();
