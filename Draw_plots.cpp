...
public:
    void initialize() override {
        t->Add("test/validation_data_CA_lundplane_parton10Mz00pt200_matchfull.root/DataInfo");
    ...
    void analyze() override {
        bool spinon = events->spin;
        bool spinoff = events->spin==0;

        bool isqq_Parton = events->isqq_Parton;
        bool isgg_Parton = events->isgg_Parton;
        bool isrest_Parton = (events->isgg_Parton + events->isqq_Parton)==0;
        bool ktcut_Parton = events->kt2_Parton > 4;
        bool zcutktcut_Parton = ktcut_Parton * (events->z1_Parton>0.2)*(events->z2_Parton>0.4);
        double phi_Parton = events->phi_Parton;

        bool isqq_Hadron = events->isqq_Hadron;
        bool isgg_Hadron = events->isgg_Hadron;
        bool isrest_Hadron = (events->isgg_Hadron + events->isqq_Hadron)==0;
        bool ktcut_Hadron = events->kt2_Hadron > 4;
        bool zcutktcut_Hadron = ktcut_Hadron * (events->z1_Hadron>0.2)*(events->z2_Hadron>0.4);
        double phi_Hadron = events->phi_Hadron;
