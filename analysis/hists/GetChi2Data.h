...
{
public:
  GetChi2Data(TH2D *input_off, TH2D *input_on, TH2D *input_data)
  {
    phi_score_on = input_on;
    phi_score_off = input_off;
    phi_score_data = input_data;
    max_chi2 = -1.0;
  ...
  TGraph Chi2_scan()
  {
    ...
    for (int i = 1; i <= bins; i++)
    {
      ...
      TH1D *phi_data = phi_score_data->ProjectionX("phi_data", i, bins);
      std::vector<TH1D *> phis = {phi_off, phi_on, phi_data};
      Plots::Histdispose hists;
      hists.rebin(phis, 10);
      hists.normalize(phis);
      std::vector<double> chi2s = hists.chi_square(phis, phi_data);
      ...
      delete phi_off, phi_on, phi_data;
    }
    return *graph;
  }
...
private:
  TH2D *phi_score_on, *phi_score_off, *phi_score_data;
  double max_chi2 = -1.0;
