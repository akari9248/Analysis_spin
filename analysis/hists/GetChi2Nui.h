...
{
public:
  GetChi2Nui(TH2D *input_off_herwig, TH2D *input_on_herwig, TH2D *input_off_pythia, TH2D *input_on_pythia)
  {
  ...
  TGraph Chi2_scan(TString mode = "off")
  {
    ...
    for (int i = 1; i <= bins; i++)
    {
      ...
      std::vector<TH1D *> phis = {herwig_off, pythia_off, herwig_on, pythia_on};
      ...
      if (mode == "off")
      {
        ...
        chiscan::chiscan chi2(herwig_on, chiscan::chiscan::GetStandarCov(herwig_on), herwig_off, {up}, {dn});
        chi2.HiddenOutput();
        chi2.scanchi2();
