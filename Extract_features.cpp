...
public:
    ...
    void initialize() override {
        treeEvents.resize(2);
        t->Add("test/pythia_pt450_"+(TString)flav+"_May28/Chunk*.root/DataInfo");
        for (int i = 0; i < 2; i++) {
    ...
    void analyze() override {
        ...
        for(int i=0;i<plane_num;i++){
            ...
            if(events->pt2_Hadron->at(i)>120||events->pt2_Hadron->at(i)<80) continue;

            int isqq=events->isqq_Hadron->at(i);
            int isgg=events->isgg_Hadron->at(i);
            JetObservable JetObservable2(jet2s.at(i),particles2.at(i));
            auto width2 = JetObservable2.jetwidth();
            JetObservable JetObservable3(jet3s.at(i),particles3.at(i));
            auto width3 = JetObservable3.jetwidth();
            JetObservable JetObservable4(jet4s.at(i),particles4.at(i));
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0;i<nchunks;i++){
        ...
        SaveData::writeTreeEventsToRoot(analyzer.treeEvents.at(0),
                                    "/extdisk/zlin/Machine_learning/test/features2_May28_pt450/j2pt80_120/Features_Spinoff"+std::to_string(i)+".root", true);
        SaveData::writeTreeEventsToRoot(analyzer.treeEvents.at(1),
                                    "/extdisk/zlin/Machine_learning/test/features2_May28_pt450/j2pt80_120/Features_Spinon"+std::to_string(i)+".root", true);
    }
