...
public:
  ...
  void analyze() override {
    ...
    double score0_2 = events->score0/(events->score0*score2_weight[0]+events->score1*score2_weight[1]+events->score2*score2_weight[2]+events->score3*score2_weight[3]);
    double score1_2 = events->score1/(events->score0*score2_weight[0]+events->score1*score2_weight[1]+events->score2*score2_weight[2]+events->score3*score2_weight[3]);
    double score3_2 = events->score3/(events->score0*score2_weight[0]+events->score1*score2_weight[1]+events->score2*score2_weight[2]+events->score3*score2_weight[3]);
    ...
    if(maxValueIndex({score0_2,score1_2,score2,score3_2})==2) {
      isqq =true;
    }
    ...
    if(isqq){
      ...
      hists["Nparticles_qq"]->Fill(events->Nparticles_3);
      hists["Nparticles_qq"]->Fill(events->Nparticles_4);
      hists["Ntracks_qq"]->Fill(events->Ntracks_3);
      hists["Ntracks_qq"]->Fill(events->Ntracks_4);
    }else if(isgg){
      hists["phi_gg"]->Fill(events->Phi);
      hists["theta_gg"]->Fill(events->theta);
      hists["theta2_gg"]->Fill(events->theta2);
      hists["Nparticles_gg"]->Fill(events->Nparticles_3);
      hists["Nparticles_gg"]->Fill(events->Nparticles_4);
      hists["Ntracks_qq"]->Fill(events->Ntracks_3);
      hists["Ntracks_qq"]->Fill(events->Ntracks_4);
    }
  }
  int maxValueIndex(const std::vector<double>& vec) {
    if (vec.empty()) {
