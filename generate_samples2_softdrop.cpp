...
LoadData(TChain &chain, int label, int entries) {
  ...
  for (int entry = 0; entry < std::min(entries, (int)chain.GetEntries());
    ...
    for (auto &jet : jets) {
      ...
      std::vector<PseudoJet> constituents = groomed_jet.constituents();
      ...
      while (j0.has_parents(j1, j2)) {
        if (j1.pt2() > j2.pt2() && j1.has_parents(j3, j4)) {
          ...
          j0 = j1;
        } else if (j1.pt2() < j2.pt2() && j2.has_parents(j3, j4)) {
...
int main() {
  ...
  // 存储到CSV文件
  std::ofstream csv_file("combined_data_CA_softdrop.csv");
  if (!csv_file.is_open()) {
