...
public:
    ...
    void analyze() override {
        ...
        int plane_num = events->pt5_Parton->size();
        ...
        for(int i=0;i<plane_num;i++){
           ParticleInfo particle( events->pt1_Parton->at(i),
                                  events->eta1_Parton->at(i),
                                  events->phi1_Parton->at(i),
                                  events->e1_Parton->at(i));
                                  
            jet1s.at(i)=particle;
        }
        for(int i=0;i<plane_num;i++){
           ParticleInfo particle( events->pt2_Parton->at(i),
                                  events->eta2_Parton->at(i),
                                  events->phi2_Parton->at(i),
                                  events->e2_Parton->at(i));
                                  
            jet2s.at(i)=particle;
        }
        for(int i=0;i<plane_num;i++){
           ParticleInfo particle( events->pt3_Parton->at(i),
                                  events->eta3_Parton->at(i),
                                  events->phi3_Parton->at(i),
                                  events->e3_Parton->at(i));
                                  
            jet3s.at(i)=particle;
        }
        for(int i=0;i<plane_num;i++){
           ParticleInfo particle( events->pt4_Parton->at(i),
                                  events->eta4_Parton->at(i),
                                  events->phi4_Parton->at(i),
                                  events->e4_Parton->at(i));
            jet4s.at(i)=particle;
        }
        for(int i=0;i<plane_num;i++){
           ParticleInfo particle( events->pt5_Parton->at(i),
                                  events->eta5_Parton->at(i),
                                  events->phi5_Parton->at(i),
                                  events->e5_Parton->at(i));
                                  
            jet5s.at(i)=particle;
        }
        for(int i=0;i<plane_num;i++){
           ParticleInfo particle( events->pt6_Parton->at(i),
                                  events->eta6_Parton->at(i),
                                  events->phi6_Parton->at(i),
                                  events->e6_Parton->at(i));
            jet6s.at(i)=particle;
        }
        for(int i=0;i<events->particle1_jetid_Parton->size();i++){
            int jetid = events->particle1_jetid_Parton->at(i);
            if(isoneGeVCut&&events->particle1_pt_Parton->at(i)<1) continue;
            ParticleInfo particle(events->particle1_pid_Parton->at(i),
                                  events->particle1_charge_Parton->at(i),
                                  events->particle1_pt_Parton->at(i),
                                  events->particle1_eta_Parton->at(i),
                                  events->particle1_phi_Parton->at(i),
                                  events->particle1_e_Parton->at(i));
            particles1.at(jetid).push_back(particle);
        }
        for(int i=0;i<events->particle2_jetid_Parton->size();i++){
            int jetid = events->particle2_jetid_Parton->at(i);
            if(isoneGeVCut&&events->particle2_pt_Parton->at(i)<1) continue;
            ParticleInfo particle(events->particle2_pid_Parton->at(i),
                                  events->particle2_charge_Parton->at(i),
                                  events->particle2_pt_Parton->at(i),
                                  events->particle2_eta_Parton->at(i),
                                  events->particle2_phi_Parton->at(i),
                                  events->particle2_e_Parton->at(i));
            particles2.at(jetid).push_back(particle);
        }
        for(int i=0;i<events->particle3_jetid_Parton->size();i++){
            int jetid = events->particle3_jetid_Parton->at(i);
            if(isoneGeVCut&&events->particle3_pt_Parton->at(i)<1) continue;
            ParticleInfo particle(events->particle3_pid_Parton->at(i),
                                  events->particle3_charge_Parton->at(i),
                                  events->particle3_pt_Parton->at(i),
                                  events->particle3_eta_Parton->at(i),
                                  events->particle3_phi_Parton->at(i),
                                  events->particle3_e_Parton->at(i));
            particles3.at(jetid).push_back(particle);
        }
        for(int i=0;i<events->particle4_jetid_Parton->size();i++){
            int jetid = events->particle4_jetid_Parton->at(i);
            if(isoneGeVCut&&events->particle4_pt_Parton->at(i)<1) continue;
            ParticleInfo particle(events->particle4_pid_Parton->at(i),
                                  events->particle4_charge_Parton->at(i),
                                  events->particle4_pt_Parton->at(i),
                                  events->particle4_eta_Parton->at(i),
                                  events->particle4_phi_Parton->at(i),
                                  events->particle4_e_Parton->at(i));
            particles4.at(jetid).push_back(particle);
        }
        for(int i=0;i<events->particle5_jetid_Parton->size();i++){
            int jetid = events->particle5_jetid_Parton->at(i);
            if(isoneGeVCut&&events->particle5_pt_Parton->at(i)<1) continue;
            ParticleInfo particle(events->particle5_pid_Parton->at(i),
                                  events->particle5_charge_Parton->at(i),
                                  events->particle5_pt_Parton->at(i),
                                  events->particle5_eta_Parton->at(i),
                                  events->particle5_phi_Parton->at(i),
                                  events->particle5_e_Parton->at(i));
            particles5.at(jetid).push_back(particle);
        }

        for(int i=0;i<events->particle6_jetid_Parton->size();i++){
            int jetid = events->particle6_jetid_Parton->at(i);
            if(isoneGeVCut&&events->particle6_pt_Parton->at(i)<1) continue;
            ParticleInfo particle(events->particle6_pid_Parton->at(i),
                                  events->particle6_charge_Parton->at(i),
                                  events->particle6_pt_Parton->at(i),
                                  events->particle6_eta_Parton->at(i),
                                  events->particle6_phi_Parton->at(i),
                                  events->particle6_e_Parton->at(i));
        ...
        for (int i = 0; i < plane_num; i++)
        {
            ...
            double jesscale = 1;
            if (!Selection::isWithinRange(events->jpt_Parton->at(i) * jesscale, jinit_range))
                continue;
            int isqq = events->isqq_Parton->at(i);
            int isgg = events->isgg_Parton->at(i);
            int isqq2 = events->isqq2_Parton->at(i);
            int isgg2 = events->isgg2_Parton->at(i);
            ...
            int type = determineType(events->flavour3_Parton->at(i), events->flavour4_Parton->at(i));
            int type3 = determineType(events->flavour5_Parton->at(i), events->flavour6_Parton->at(i));

            if(type==0 && type3==0 ) continue;
            if(events->kt2_Parton->at(i)<2 || events->kt3_Parton->at(i)<2 ) continue;
            ...
            treeEvents.assign("pt", events->jpt_Parton->at(i));
            treeEvents.assign("z1", events->z1_Parton->at(i));
            treeEvents.assign("z2", events->z2_Parton->at(i));
            treeEvents.assign("z3", events->z3_Parton->at(i));

            treeEvents.assign("DeltaR", events->deltaR2_Parton->at(i));
            treeEvents.assign("DeltaR3", events->deltaR3_Parton->at(i));
            treeEvents.assign("kt", events->kt2_Parton->at(i));
            treeEvents.assign("kt0", events->kt1_Parton->at(i));
            treeEvents.assign("kt3", events->kt3_Parton->at(i));
            treeEvents.assign("Phi",events->phi_Parton->at(i));
            treeEvents.assign("Phi3",events->thirdphi_Parton->at(i));
            treeEvents.assign("type",type);
            treeEvents.assign("type3",type3);
            treeEvents.assign("match",0);
