...
def main(args):
    ...
    select_item = ["pt3", "eta3", "phi3", "e3", "pt4", "eta4", "phi4", "e4", "pt5", "eta5", "phi5", "e5", "z1", "z2", "kt1", "kt2", "score0", "score1", "score2", "score3"]
    # select_item = ["pt3", "eta3", "phi3", "pt4", "eta4", "phi4", "pt5", "eta5", "phi5", "z1", "z2", "kt1", "kt2", "score0", "score1", "score2", "score3"]
    ...
    for iter in range(iter_num):
        ...
        if mode == "Train":
            ...
            Y_data0 = dnn.convert_to_one_hot_twolabel_spin(Y_data, match_data, kt2_feature, spin_label)
        ...
        if mode == "Prediction":
            ...
            # basefolder_name = os.path.basename(dir_path)+"/"   
            folder_name = '/storage/shuangyuan/code/analysis_spin/Machine_learning/ML/Datasets_new/Prediction_ak8_spin_bdt/'+basefolder_name
            ...
            for level_prefix in level_prefixs:
                for level_suffix in level_suffixs:
                    ...
                    dtest = xgb.DMatrix(X_data_predict)
                    full_predictions = model.predict(dtest)
                    if len(full_predictions.shape) == 1:
                        n_classes = 2  # 假设二分类
                    else:
                        n_classes = full_predictions.shape[1]  # 多分类
                    print(full_predictions)
                    full_predictions = np.stack([full_predictions, 1 - full_predictions], axis=1)
                    print(full_predictions)
            ...
            if len(args.train_val_index_full)>0:
                ...
                # print("Length of train_val_index:", len(train_val_index))
            ...
            # print("Length of args.train_val_index_full:", len(args.train_val_index_full))
    ...
    if mode == "Train":
        ...
        l2_reg=0
        model = train_and_save_model_bdt(X_train=X_data1_train_full, Y_train=Y_data1_train_full, X_val=X_data1_val_full, Y_val=Y_data1_val_full, hidden_units=[16], learning_rate=0.0001, l2_reg=0)
        model_folder = "ML/model/Fourlabel1_"+suffix
        if not os.path.exists(model_folder):
            os.makedirs(model_folder)
        model.save_model(model_folder+"/bdt.json")
        # model.save("ML/model/Fourlabel1_"+suffix+"/bdt_model.json")
        ######################## roc curve ##############################
        ...
        args.model_path = model_folder+"/bdt.json"
